...
from cmdlang import codigo, interp, semantico, sintatico
...
def testa_validos(usar_gcc):
    falhas = []
    arquivos = sorted(f for f in os.listdir(DIR_OK) if f.endswith(".cmd"))
    for nome in arquivos:
        ...
        print("  %-28s %-8s %s" % (nome, status, "; ".join(detalhes)))
        if detalhes:
            falhas.append(nome)
...
def testa_erros():
    falhas = []
    arquivos = sorted(f for f in os.listdir(DIR_ERRO) if f.endswith(".cmd"))
    for nome in arquivos:
