#!/usr/bin/env python3
"""Compilador da linguagem Cmd. Uso: python3 cmdc.py <acao> <fonte.cmd>"""
...
from cmdlang import ast, codigo, interp, lexico, semantico, sintatico
from cmdlang.erros import ErroCmd

ACOES = ("tokens", "ast", "check", "asm", "build", "run", "interp")

USO = """uso: python3 cmdc.py <acao> <fonte.cmd> [saida]
...
def principal(argv):
    ...
    try:
        ...
        if acao == "check":
            print("ok: %d variaveis, %d comandos" % (len(tab), len(prog.cmds)))
            return 0
