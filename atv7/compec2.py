#!/usr/bin/env python3
"""Compilador completo da linguagem EC2 (Expressoes Constantes 2).

Le um arquivo de entrada com uma expressao EC2, realiza a analise lexica e
...
Opcoes auxiliares:
    ...
    --otimizar  aplica propagacao de constantes: como toda expressao EC2 e
                constante, emite um unico "mov $<resultado>, %rax".

A AST usa os mesmos nos da Atividade 06; a novidade desta atividade e a
gramatica de expressoes com precedencia e associatividade a esquerda.
...
class Lexer:
    ...
    def proximo_token(self) -> Token:
        ...
        Levanta ErroLexico se encontrar um caractere fora do alfabeto EC2.
        """
        self._pular_espacos()
...
class Parser:
    ...
    def _analisa_exp_m(self) -> Exp:
        """Analisa expressoes multiplicativas.
...
MODELO = """\
  #
  # codigo gerado pelo compilador EC2
  #
  .section .text
  .globl _start
...
def gerar_assembly(arvore: Exp, otimizar: bool = False) -> str:
    ...
    Com otimizar=True aplica propagacao de constantes (toda expressao EC2 e
...
def _analisar_argumentos(argv: list) -> tuple:
    ...
    while i < len(args):
        ...
        if a == "-o":
            i += 1
            if i >= len(args):
                print(f"uso: {argv[0]} <arquivo.ec2|arquivo.ec1> [-o saida.s] [--otimizar]",
                      file=sys.stderr)
                sys.exit(2)
            saida = args[i]
        elif a == "--otimizar":
